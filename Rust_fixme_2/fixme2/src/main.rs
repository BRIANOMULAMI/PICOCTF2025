...
fn decrypt(encrypted_buffer: Vec<u8>, borrowed_string: &mut String) { // FIXED: Use &mut String
    ...
    let res = XORCryptor::new(&key);
    if res.is_err() {
        return; // Correct way to return in Rust
    }
    let xrc = res.unwrap();
...
fn main() {
    ...
    let mut party_foul = String::from("Using memory unsafe languages is a: "); // FIXED: `mut` keyword
